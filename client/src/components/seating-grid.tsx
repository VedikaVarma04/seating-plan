          ...
          {rowArr.map((row) => (
            <div key={`row-${row}`} className="flex justify-center gap-4 md:gap-6">
              ...
              {colArr.map((col) => {
                ...
                const isOccupied = !!seat?.studentRollNo;
                const isHighlighted =
                  highlightRollNo &&
                  seat?.studentRollNo?.toUpperCase() === highlightRollNo.toUpperCase();
                  ...
                  <div key={`cell-${row}-${col}`} className="relative group">
                    <button
                      ...
                      className={cn(
                        ...
                        isHighlighted &&
                          "ring-4 ring-primary ring-offset-2 bg-[#FFF9C4] border-primary",
                        readOnly && !isOccupied && "opacity-50"
                      )}
                    >
                      {isOccupied ? (
                        <div className="flex flex-col items-center p-1 w-full h-full justify-between">
                          <div className="text-[10px] font-bold text-muted-foreground self-start px-1 uppercase tracking-tighter">
                            {seat?.studentRollNo}
                          </div>
                          ...
                          {seat?.studentName && (
                            <div className="text-[8px] font-medium text-muted-foreground self-end px-1 italic truncate max-w-[60px]">
                              {seat.studentName}
                            </div>
                          )}
                        </div>
                    ...
                    {!readOnly && seat && (
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-popover text-popover-foreground text-xs rounded shadow-lg border opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                        Row {row + 1} Col {col + 1}
                        {seat.studentRollNo && ` • ${seat.studentRollNo}`}
                      </div>
                    )}
                  </div>
                );
